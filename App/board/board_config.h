...
/* Wagan: 2026-07-26 — ЭКСПЕРИМЕНТ: сырой съём ВСЕГО аккумулятора CIR по РАННЕМУ
 * событию приёма (RXSFDD/RXPHE/RXFCE), не дожидаясь зачёта кадра (RXFCG). Проверка
 * гипотез будущего 5-канального обнаружителя (HANDOFF §15.3), пока его плата не
 * готова. НЕ фронт МКС, НЕ функция продукта. Основание — docs\RECON_raw_cir_monitor.md
 * (Б1: аккумулятор наполняется по детекту преамбулы, до PHR/CRC; «по таймеру без
 * сигнала» невозможно — не пытаться). Реализация — docs\TASK_raw_cir_monitor.md /
 * docs\REPORT_raw_cir_monitor.md. Раскомментировать строку ниже → добавляется команда
 * CMD_CIR_RAW_ARM (0x43) и потоковый кадр CONTENT=5 (полный сырой CIR). При
 * ЗАКОММЕНТИРОВАННОМ флаге прошивка байт-в-байт как сейчас. НЕ забыть пересобрать. */
/* #define RAW_CIR_MONITOR */

/* Wagan: 2026-08-01 — ЭКСПЕРИМЕНТ: энергоскан на АРУ (ЭД). По UM §7.2.36.2 при DIS_AM=0
 * и включённом приёмнике АРУ устаканивается на оценке фоновой энергии (EDG1/EDV2 в
 * AGC_STAT1 0x23:1E) — «Energy Scan / ED channel scan», независимо от детекта преамбулы.
 * Цель спайка — да/нет: даёт ли узел контраст пустой эфир / активность в канале. НЕ фронт
 * МКС, НЕ функция продукта. Основание — docs\RECON_ed_agc.md; реализация —
 * docs\TASK_ed_agc_impl.md / docs\REPORT_ed_agc_impl.md. Раскомментировать строку ниже →
 * добавляется команда CMD_ED_AGC_SCAN (0x44) и включение DWT CYCCNT (пауза 32 мкс).
 * НЕЗАВИСИМ от RAW_CIR_MONITOR. При ЗАКОММЕНТИРОВАННОМ флаге прошивка байт-в-байт как
 * сейчас. НЕ забыть пересобрать. */
/* #define ED_AGC_SCAN */
