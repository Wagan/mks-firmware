...
**Статус документа:** рабочий черновик v0.13 (регулировка мощности TX работает:
`SET_TX_POWER` подтверждён loopback M1→M2 на железе — RX_LEVEL монотонно растёт с
level. Ранее: приём и передача, включая периодическую
...
11. **ТЕКУЩЕЕ / остаток фронтов:** **строгий RSSI/SNR** в прошивке (нужен
    `RXPACC_NOSAT`, см. `docs/PROTOCOL_SPEC.md`); `GET_CIR` (0x41); **EXTI**
    (перекрёстные IRQ) вместо polling для приёма/событий. Далее — эксперименты и
    автоматизация (этапы ТЗ).
