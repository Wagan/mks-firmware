...
**Статус документа:** рабочий черновик v0.14 (приёмная диагностика доведена до
**строгого RSSI/FP_POWER в прошивке** — не приближённого: чтение RXPACC_NOSAT +
формулы UM §4.7 на float, сверено с хостом Δ=0.00 dB. Ранее: регулировка мощности
TX (`SET_TX_POWER`), приём и передача, включая периодическую
(`TX_FRAME`/`TX_PERIODIC`/`TX_STOP`), `SET_PHY_CONFIG` + приёмный тракт против
EVK1000 Mode 3, слой протокола PING/INIT/GET_STATUS, устранение зависания INIT в USB-ISR)
...
12. **ТЕКУЩЕЕ / остаток фронтов:** **SNR** (следующий микрошаг — δ vs `stdNoise`);
    `GET_CIR` (0x41); **EXTI** (перекрёстные IRQ) вместо polling для приёма/событий.
    Далее — эксперименты и автоматизация (этапы ТЗ).
