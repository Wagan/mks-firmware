...
**Статус документа:** рабочий черновик v0.8 (bring-up пройден: DEV_ID `0xDECA0130`
прочитан с обоих модулей — M1/SPI2 и M2/SPI3; закрыты OQ-9 и §14.4)
...
5. **ТЕКУЩЕЕ:** слой протокола (PING/INIT/GET_STATUS) → обмен с MATLAB.
6. Перенести/проверить на 2 чипа (SPI2/SPI3, перекрёстные IRQ).
7. Конфигурация/передача → приём/диагностика → автоматизация (этапы ТЗ).
