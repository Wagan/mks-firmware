...
14. **ТЕКУЩЕЕ / остаток фронтов:** **EXTI** (перекрёстные IRQ) вместо polling для
    приёма/событий; **CIR-2** (потоковый режим + `SET_STREAM_MODE`); **CIR-3**
    (утилита-водопад). Далее — эксперименты и автоматизация (этапы ТЗ).
