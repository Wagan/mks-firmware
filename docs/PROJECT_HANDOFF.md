...
**Статус документа:** рабочий черновик v0.10 (приёмный тракт работает: `SET_PHY_CONFIG`
+ `RX_START`/`RX_STOP`/`GET_SIGNAL_METRICS` реализованы и проверены на железе против
эталона EVK1000 Mode 3; RX — доверенный узел для проверки будущего TX. Ранее: слой
протокола PING/INIT/GET_STATUS, устранено зависание INIT в USB-ISR)
...
8. **ТЕКУЩЕЕ:** TX-тракт (`TX_FRAME` 0x20 / `TX_STOP` 0x22 / `TX_PERIODIC` 0x21)
   + `SET_TX_POWER` (0x11). Даёт собственный источник кадров (M1→M2), проверяемый
   против уже доверенного RX. Далее — диагностика (RSSI/SNR, CIR), автоматизация.
9. Перенести/проверить на 2 чипа для TX (SPI2/SPI3, перекрёстные IRQ). INIT/RX
   уже работают на модулях; остаётся TX-тракт и (позже) обработка IRQ через EXTI.
