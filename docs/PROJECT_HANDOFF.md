...
**Статус документа:** рабочий черновик v0.7 (добавлен §14 питание/безопасность; §11 актуализирован)
...
4. **ТЕКУЩЕЕ:** прочитать `DEV_ID` (ожидаем `0xDECA0130`). Отладка от ST-LINK v2
   mini срывалась (errno 10061) — вероятно, нехватка питания; решается внешним
   питанием платы (см. §14). ВНИМАНИЕ: на испытываемой плате распаян только ОДИН
   модуль DWM1000 — второй ответит «нет», это нормально.
5. Слой протокола (PING/INIT/GET_STATUS) → обмен с MATLAB.
...
### 14.4 Симптом отладки errno 10061 (target disconnected)
При отладке от ST-LINK v2 mini (плата запитана ОТ программатора) связь рвётся
(`Remote communication error, target disconnected, errno 10061`) — вероятно,
нехватка тока от mini-ST-LINK, когда МК начинает исполнять код. Решение — внешнее
питание платы. Проверить после подведения внешнего 3.3В.

---

*Документ будет дополняться по мере поступления данных (результат чтения DEV_ID
после внешнего питания, MATLAB-скрипт заказчика).*
