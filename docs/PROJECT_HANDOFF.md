...
**Статус документа:** рабочий черновик v0.9 (слой протокола работает: команды
`PING`/`INIT`/`GET_STATUS` проверены end-to-end на железе через `tools/mks_*_test.py`
по USB CDC / COM3; устранено зависание INIT в USB-ISR — разбор вынесен в main loop)
...
6. **ТЕКУЩЕЕ:** конфигурация PHY — `SET_PHY_CONFIG`/`SET_TX_POWER` (трансляция
   wire→enum DecaDriver, §4.1) и далее тракт TX/RX/диагностики.
7. Перенести/проверить на 2 чипа для TX/RX (SPI2/SPI3, перекрёстные IRQ). INIT
   уже инициализирует оба модуля; остаётся TX/RX-тракт и обработка IRQ.
8. Конфигурация/передача → приём/диагностика → автоматизация (этапы ТЗ).
