...
**Статус документа:** рабочий черновик v0.12 (приём и передача, включая
периодическую, работают: `TX_FRAME`/`TX_PERIODIC`/`TX_STOP` подтверждены внутренним
loopback M1→M2 на железе — замкнута доверенная пара свой TX ↔ свой RX. Ранее:
...
10. **ТЕКУЩЕЕ / остаток фронтов:** `SET_TX_POWER` (0x11); **строгий RSSI/SNR** в
    прошивке (нужен `RXPACC_NOSAT`, см. `docs/PROTOCOL_SPEC.md`); `GET_CIR` (0x41);
    **EXTI** (перекрёстные IRQ) вместо polling для приёма/событий. Далее —
